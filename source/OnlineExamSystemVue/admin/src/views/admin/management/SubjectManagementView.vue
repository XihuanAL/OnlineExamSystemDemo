...
export default {
  ...
  methods: {
    getSubjectList() {
      ...
      this.$axios.get("/subjects/" + this.pagination.currentPage + "/" + this.pagination.pageSize + param).then((res) => {
    ...
    handleAddSubject() {
      this.$axios.post("/subjects", this.formData).then((res) => {
        this.dialogFormVisible = false;
        this.getSubjectList();
      })
    },
    handleEditSubject() {
      this.$axios.put("/subjects", this.formData).then((res) => {
        this.dialogFormVisible4Edit = false;
        this.getSubjectList();
      })
    },
    editSubject(row) {
      ...
      this.$axios.get("/subjects/" + row.id).then((res) => {
    ...
    delSubject(row) {
      this.$confirm('此操作将同时删除该学科下的所有题目和试卷, 是否继续?', '提示', {
        ...
        this.$axios.delete("/subjects/" + row.id).then((res) => {
          this.getSubjectList();
        });
      }).catch(() => {
      });
